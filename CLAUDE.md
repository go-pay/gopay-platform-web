...
> **最后更新**: 2026-04-09
>
> **当前阶段**: 前后端联调完成 → 功能测试 & 优化
>
> **阻塞项**: 无

### 快速了解

前端全部 18 个页面已完成与后端 API 的对接（67 个接口）。项目采用集中式 API 层架构（`src/api/`），所有页面从 Mock 数据切换为真实后端调用。

### API 层架构

```
src/api/
├── request.ts          # 基础请求封装（token 注入、401 处理、文件下载）
├── auth.ts             # 登录/用户/密码/资料
├── dashboard.ts        # 仪表盘
├── merchant.ts         # 商户管理
├── merchantApp.ts      # 商户应用
├── incoming.ts         # 进件管理 + 图片上传
├── channel.ts          # 支付通道
├── order.ts            # 支付/退款/转账订单
├── transaction.ts      # 交易流水/回调
├── reconciliation.ts   # 对账单/差异
└── system.ts           # 用户/角色/日志
```
...
| 优先级 | 任务 | 状态 |
|--------|------|------|
| P0 | 全模块功能测试（逐页面验证 CRUD + 边界情况） | 待测试 |
| P1 | 错误提示优化（替换 alert 为 toast/snackbar） | 待开发 |
| P1 | 分页组件统一（当前各页面分页实现不一致） | 待开发 |
| P2 | 权限控制（根据用户 role 控制菜单和按钮可见性） | 待开发 |
| P3 | 性能优化（dashboard ECharts 打包体积过大） | 待优化 |
