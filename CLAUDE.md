...
**重要**: 在开始任何开发工作前：
1. 阅读下方「项目进度」了解当前阶段和待办事项
2. 查看 `docs/CHANGELOG.md` 了解最近的改动历史，避免重复工作或引入冲突

## 开发后必做

**每次完成开发工作后，必须同步更新以下文件**：
1. **CLAUDE.md** — 更新「项目进度」章节中的当前阶段、已完成项、待办事项
2. **docs/CHANGELOG.md** — 记录本次改动的详情（问题描述、解决方案、修改的文件）

这是项目的交接文档，确保在任何电脑、任何 Claude 会话中都能快速接手当前工作。
...
> **当前阶段**: 前端开发完成 → 等待后端联调
>
> **阻塞项**: 后端 gopay-platform 各模块 API 尚未就绪

### 快速了解

前端全部 18 个页面 UI 及交互已完成，均使用 Mock 数据。项目暂停前端开发，等待后端 API 就绪后逐模块对接。

### 接下来要做什么

当后端 API 就绪后，按以下优先级逐模块联调（替换 Mock 为真实 API 调用）：

| 优先级 | 模块 | 关键接口 | 状态 |
|--------|------|----------|------|
| P0 | 登录/SSO | `POST /gopay/v1/sso/login` | 待后端开发 |
| P0 | 用户信息 | `POST /gopay/v1/user/getInfo` | 待后端开发 |
| P1 | 商户管理 | 商户列表 & 商户应用 CRUD | 待后端开发 |
| P2 | 支付配置 | 通道管理 CRUD | 待后端开发 |
| P3 | 订单中心 | 支付/退款/转账订单查询 | 待后端开发 |
| P4 | 进件管理 | 进件申请 & 记录 | 待后端开发 |
| P4 | 交易记录 | 交易流水 & 回调记录 | 待后端开发 |
| P4 | 对账管理 | 对账单 & 差异记录 | 待后端开发 |
| P4 | 系统管理 | 用户/角色/日志 | 待后端开发 |
...
- `npm run build` 有 TS 类型检查报错（非新引入），日常构建用 `npx vite build` 绕过
- 图片上传仅 dataURL 预览，无真实上传逻辑
