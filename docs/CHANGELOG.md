...
**里程碑**: 全部 18 个页面从 Mock 数据切换为真实后端 API 调用，共对接 67 个接口。

**架构改动**:
- 新建 `src/api/request.ts` — 统一请求封装（token 注入、401 自动跳登录、错误处理）
- 新建 10 个 API 模块文件: `auth.ts`, `dashboard.ts`, `merchant.ts`, `merchantApp.ts`, `channel.ts`, `incoming.ts`, `order.ts`, `transaction.ts`, `reconciliation.ts`, `system.ts`
- 改造 `src/store/user.ts` — 使用 API 层替代 raw fetch

**页面改动（17 个 .vue 文件）**:
- 删除所有硬编码 Mock 数据
- 添加 `onMounted` 生命周期加载真实数据
- 增删改操作改为 API 调用 + 刷新列表
- 金额处理: API 传输用分(int64)，前端展示 /100 转元
- 添加服务端分页状态（page, pageSize, total）
- 图片上传改为真实文件上传（返回 CDN URL）
